    ...
    return this.callHook([ 'save', 'create' ], obj, (modifiedObj) => {
      return this.describeSobject()
        .then((table) => {
          ...
          //   if (modifiedObj.hasOwnProperty(key) && (!field.creatable || field.deprecated || field.error)) {
          //     if (!this[_ignoreErrors]) {
          //       throw new SalineError(`Unable to create field: ${field.name} ${(field.error ? field.error.message : '')}`);
          //     }
          ...
          //   if (modifiedObj.hasOwnProperty(key) || (field.required && (!field.nillable && field.defaultValue !== null))) {
    ...
    return this.callHook([ 'save', 'update' ], obj, (modifiedObj) => {
      return this.describeSobject()
        .then((table) => {
          ...
          // obj = objectReduce(describe.fields, (newObj, field, key) => {
          //   if (modifiedObj.hasOwnProperty(key) && (key !== describe.idField && !field.updatable || field.deprecated || field.error)) {
          //     if (!this[_ignoreErrors]) {
          //       throw new SalineError(`Unable to update field: ${field.name} ${(field.error ? field.error.message : '')}`);
          //     }
